...
function queryDelete() {
	...
	$('#dataTable input.check-all').attr('checked', false);
...
function queryAddRecord() {
	...
	setTimeout(function() { bottom = $("#results-div").attr("scrollHeight"); $("#results-div").attr("scrollTop", bottom);}, 50);
}
...
function resultSelectAll() {
	check = $('#dataTable input.check-all').attr('checked');
	$('#dataTable input').attr('checked', check);
...
function setupResults() {
	...
	if (editTableName != "")
		$('#dataTable input').not('check-all').click(function() { showNavBtn('delete', 'gensql'); });
	
	//$("#dataTable").contextMenu(getDataMenu);
}

function postSortTable() {
//	$('#dataTable input:checked').attr('checked', false);
}

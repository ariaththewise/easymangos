/**********************************************************
 *  editable.js - Author: Samnan ur Rehman                *
 *  This file is a part of MyWebSQL package               *
 *  collection of functions to help with tables editing   *
 **********************************************************/
...
function addField(n) {
	for (i=0; i<n; i++) {
		...
		if (selectedRow != null)
			$('#table_grid tbody tr.sel').after(rows);
		else
...
function closeEditor(type, upd, forward) {
	...
	if (upd) {
		if (type == 'combo') {
			...
			txt = obj.find('input').attr('checked') ? checkBoxCode : '';
			obj.html(txt);
		} else {
